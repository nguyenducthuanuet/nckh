...
//		syncReturnFlag();
	...
	private void makeFormula() {
		...
		if ( fElse.hasChild() )
			fElse.setCondition( wrap("not", ifFlagStr) );
	...
	private Formula fThen;	// formula of then statement
	private Formula fElse;  	// formula of else statement
	
	private Formula thenAddition = new Formula();	// phan them vao bieu thuc then nhu: return, break, continue hay dong bo gia tri cac bien
	private Formula elseAddition = new Formula();	// phan them vao bieu thuc else nhu: return, break, continue hay dong bo gia tri cac bien
