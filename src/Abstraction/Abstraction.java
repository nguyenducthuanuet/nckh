...
//			System.out.println("parent: " + unaryOp.getParent());
			// phep toan mot ngoi nam trong phep toan hai ngoi
			// chi tra ve gia tri chua bien khi thuc hien phep toan
			if(unaryOp.getParent() instanceof CtBinaryOperator || 
				unaryOp.getParent() instanceof CtAssignment) {
				// neu la tang hoac giam truoc(++i, --i) thi tra ve gia tri sau khi thuc hien phep toan
				if(operator == UnaryOperatorKind.PREDEC || operator == UnaryOperatorKind.PREINC) {
					variable.increase();
					return variable.getValue();
				} else {	// tra ve gia tri truoc khi thuc hien phep toan
	...
	public List<String> formularize(CtIf ifs, List<Variable> listVariables) {
		...
		if(elseCtStatement == null) {
			...
			Helper.syncIndex(listVariables, thenList);
			return f;
		}
		...
		String thenUpdate = updateVariable(thenList, listVariables);
		if(thenUpdate != null)
			fThen = wrap(fThen, "and", thenUpdate);
		String elseUpdate = updateVariable(elseList, listVariables);
		if(elseUpdate != null)
	...
	public List<String> formularize(CtAssignment ass, List<Variable> listVariables) {
		...
		Filter filter = new TypeFilter(CtUnaryOperator.class);
		List<CtUnaryOperator> unarys = ass.getElements(filter);
		CtElement parent;
		for(CtUnaryOperator u: unarys) {
			System.out.println("u: " + u);
			List<CtLiteral> temp = u.getElements(new TypeFilter(CtLiteral.class));
			if(temp == null) {
				System.out.println("formularize");
				parent = u.getParent();
				u.setParent(null);
				f.add(formularize(u, listVariables));
				u.setParent(parent);
			}
			else if(temp.size() == 0) {
				System.out.println("formularize");
				parent = u.getParent();
				CtElement parentTemp = new CtElementImpl() {
